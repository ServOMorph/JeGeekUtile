...
def _create_test_users():
    ...
    if not test_user:
        ...
        db.session.commit()
        print(f'✅ Test account created: {test_email} / TestPassword123')
    else:
        print(f'ℹ️  Test account already exists: {test_email}')
    ...
    if not admin_user:
        ...
        db.session.commit()
        print(f'✅ Admin account created: {admin_email} / AdminPassword123')
    else:
    ...
    if not presentation_app:
        ...
        db.session.commit()
        print(f'✅ App created: Présentation')
    ...
    if test_user and presentation_app and presentation_app.id not in (test_user.installed_apps or []):
        ...
        db.session.commit()
        print(f'✅ App installed for test user: Présentation')
    ...
    if admin_user and presentation_app and presentation_app.id not in (admin_user.installed_apps or []):
        ...
        db.session.commit()
        print(f'✅ App installed for admin user: Présentation')
